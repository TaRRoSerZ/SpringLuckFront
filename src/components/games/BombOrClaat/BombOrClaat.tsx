...
export default function BombOrClaat() {
  ...
  function handleTileClick(index: number) {
    if (!isGameActive || !betPlaced) return;
    ...
    if (!newTiles[index].isBomb) {
      ...
      // Bomb touchée : game over, pas de gain
      setGameOver(true);
  ...
  function handleChangeLevel(level: Level) {
    ...
    if (isGameActive) return; // Empêcher le changement pendant une partie
    setActiveLevel(level.name);
    setNumberOfBomb(level.numberOfBomb);
  }

  async function handleCashOut() {
    if (!isGameActive || safePicks === 0) return;
    const winAmount = betAmount * multiplier;
    await recordWin(winAmount);
      ...
      <section className="bomb-or-claat-game">
        ...
        <div className="bomb-or-claat-settings-tab">
          ...
          <div className="bomb-or-claat-settings-actions">
            ...
            <button
              className="bomb-or-claat-cash-out-button"
              onClick={handleCashOut}
              disabled={!isGameActive || safePicks === 0}>
              Cash out
            </button>
          </div>
        </div>
