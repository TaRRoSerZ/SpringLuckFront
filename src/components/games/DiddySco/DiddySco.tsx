...
const DiddySco = () => {
	...
	// Bet / balance state (mirror BombOrClaat behavior)
	...
	function checkWin() {
		...
		if (newWinningSymbols.length > 0) {
			...
			// compute payout: betAmount * winMultiplier
			const payout = Number((betAmount * totalMultiplier).toFixed(2));
	...
	const handleSpin = () => {
		if (spinning) return;
		if (!betPlaced || betAmount <= 0) return; // require bet
		setWinSymbol(null);
		setWinMultiplier(0);
		...
		<>
			...
			<section className="diddy-sco">
				...
				<button
					...
					disabled={spinning || !betPlaced} // Désactive le bouton pendant l'animation ou si pas de mise
					onClick={handleSpin}
				>
